...
- **THEN** the GEMV kernels execute with adaptive occupancy grids achieving $\ge 90$ tokens per second on an NVIDIA RTX 3060 Laptop GPU
...
- **THEN** the draft model emits $K=3..5$ candidate tokens on GPU and the target model verifies candidate logits in a single batched step, yielding effective throughput $\ge 140$ tokens per second on 3B models
...
The Titan CLI executable SHALL provide `titan run <path>` for terminal chat with live markdown rendering and performance telemetry, and `titan serve <path>` for OpenAI-compatible HTTP streaming.
