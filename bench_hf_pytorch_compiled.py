...
def bench():
    ...
    print(f"Loading {model_id} in PyTorch (FP16, SDPA, torch.compile with CUDA Graphs)...")
    ...
    # Static KV-Cache + compile
    model.generation_config.cache_implementation = "static"
    model.forward = torch.compile(model.forward, mode="reduce-overhead", fullgraph=False)
    
    # Warmup
    print("Warming up compiled graph...")
    with torch.inference_mode():
        _ = model.generate(**inputs, max_new_tokens=5, do_sample=False)
        torch.cuda.synchronize()
        
    n_tokens = 41
    print(f"Generating {n_tokens} tokens with compiled PyTorch...")
    ...
    print(f"\n================================================================================")
    print(f"PyTorch Compiled (torch.compile + CUDA Graphs) Result:")
