...
fn test_radix_prefix_caching_speedup() -> Result<(), DynError> {
    ...
    println!("  [Turn 1] Full Prefill TTFT: {:.2} ms ({} tokens processed)", t1_ms, tokens_1.len());
    ...
    println!("  [Turn 2] Radix APC Cached TTFT: {:.2} ms (suffix prefill only!)", t2_ms);
    ...
    println!("  [Turn 3] 100% Cached TTFT: {:.2} ms (Instant Cache Hit!)", t3_ms);
