//! Real Qwen3 fused Gate/Up layer-0 diagnostic (GPU vs independent CPU).
...
#[test]
#[ignore]
fn real_fused_gate_up_qwen3_parity() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    assert_eq!(gate_info.ggml_type, GgmlType::Q4_K);
    assert_eq!(up_info.ggml_type, GgmlType::Q4_K);
    assert_eq!(n, 1, "this diagnostic intentionally exercises batch=1");
    println!("fused_gate_up dimensions: batch={n} hidden={h} intermediate={intermediate}");
    ...
    let qx = DeviceBuffer::alloc(d.clone(), h)?;
    let qd = DeviceBuffer::alloc(d.clone(), (h / 32) * 4)?;
    let qs = DeviceBuffer::alloc(d.clone(), (h / 32) * 4)?;
    let mut gate_cpu = vec![0.; intermediate];
    let mut up_cpu = vec![0.; intermediate];
    matmul(&mut gate_cpu, &gate, &fc);
    matmul(&mut up_cpu, &up, &fc);
    g.quantize_q8_1(&s, &norm_dev, None, &qx, &qd, &qs, h, eps)?;
    let gate_q8 = DeviceBuffer::alloc(d.clone(), intermediate * 4)?;
    let up_q8 = DeviceBuffer::alloc(d.clone(), intermediate * 4)?;
    ...
    let gate_gpu = download(&s, &gate_q8, intermediate)?;
    let up_gpu = download(&s, &up_q8, intermediate)?;
    let (gate_q8_rel_l2, gate_q8_cosine) = metric(&gate_gpu, &gate_cpu);
    println!(
        "stage=individual_gate_q8_act format=Q4_K shape=[{n},{intermediate}] batch={n} rel_l2={gate_q8_rel_l2:.6e} cosine={gate_q8_cosine:.9}"
    );
    let (up_q8_rel_l2, up_q8_cosine) = metric(&up_gpu, &up_cpu);
    println!(
        "stage=individual_up_q8_act format=Q4_K shape=[{n},{intermediate}] batch={n} rel_l2={up_q8_rel_l2:.6e} cosine={up_q8_cosine:.9}"
    );
    let q8_product = gate_gpu
        .iter()
        .zip(&up_gpu)
        .map(|(&g, &u)| {
            let x = if g > 20.0 {
        ...
        .collect::<Vec<_>>();
    let fused = DeviceBuffer::alloc(d.clone(), intermediate * 4)?;
    g.gemm_q4k_fused_gate_up_swiglu_mma(
        &s,
        &gate_dev,
        &up_dev,
        &qx,
    ...
    let gpu = download(&s, &fused, intermediate)?;
    let (fused_q8_rel_l2, fused_q8_cosine) = metric(&gpu, &q8_product);
    println!(
        "stage=fused_vs_individual_q8_product format=Q4_K shape=[{n},{intermediate}] batch={n} rel_l2={fused_q8_rel_l2:.6e} cosine={fused_q8_cosine:.9}"
    );
    assert!(
        fused_q8_rel_l2 < 2e-4 && fused_q8_cosine > 0.999999,
        "fused vs individual Q8 product failed: rel_l2={fused_q8_rel_l2:.6e} cosine={fused_q8_cosine:.9}"
    );
    let cpu = gate_cpu
        .iter()
        .zip(up_cpu)
