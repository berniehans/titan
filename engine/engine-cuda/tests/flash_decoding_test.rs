...
use engine_cuda::{DeviceBuffer, ensure_cuda_dll_paths};
...
fn test_flash_decoding_compilation_and_execution() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let head_dim = 128;
    let block_tokens = 16;
    let seq_tokens: usize = 1024; // 1024 tokens = 4 splits of 256 tokens

    let q = DeviceBuffer::alloc(dev.clone(), n_head * head_dim * 4)?;
    let floats_per_block = block_tokens * 2 * n_head_kv * head_dim;
    let num_blocks = seq_tokens.div_ceil(block_tokens);
    ...
    let bt_bytes: &[u8] =
        unsafe { std::slice::from_raw_parts(bt_host.as_ptr() as *const u8, bt_host.len() * 4) };
    ...
    stream.sync()?;
    println!(
        "Successfully compiled and executed FlashDecoding Split-KV Attention (1024 tokens) on GPU!"
    );
